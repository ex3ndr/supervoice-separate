import torch
from .model import SeparateModel
from .config import config
from .audio import spectogram

class SuperVoiceSeparate(torch.nn.Module):
    def __init__(self, flow, vocoder):
        super(SuperVoiceSeparate, self).__init__()
        self.diffusion = SeparateModel(flow, config)
        self.vocoder = vocoder
        self.sample_rate = config.audio.sample_rate

    @torch.no_grad()
    def separate(self, waveform, *, steps = 8, alpha = None):
